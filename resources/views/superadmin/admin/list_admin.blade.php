...
<head>
    @include('layouts.superadmin.partials.head', ['pageTitle' => 'Manajemen Admin — Flodemi'])
</head>
    ...
    <div class="flex">
        ...
        <div class="main-wrapper w-full flex flex-col min-h-screen" id="mainWrapper">
            ...
            <main class="flex-1 p-6 md:p-8">
                {{-- Page Header --}}
                <div class="mb-8">
                    <h2 class="text-xl md:text-2xl font-extrabold text-slate-800 dark:text-white tracking-tight">Manajemen Admin</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Kelola akun admin, atur peran, dan pantau status pengguna secara terpusat.</p>
                    ...
                    <nav class="flex items-center gap-2 text-[10px] font-bold text-slate-400 mt-3 uppercase tracking-wider">
                        ...
                        <span class="text-slate-500 dark:text-slate-300">Daftar Admin</span>
                    </nav>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="table-modern w-full">
                            ...
                            <tbody id="adminTable">
                                    ...
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors" data-id="{{ $admin->id }}" data-status="{{ $admin->status ?? 'aktif' }}">
                                        <td>
                                            <div class="flex items-center gap-3 px-6 py-4">
                                                ...
                                                @endif
                                                <a href="{{ route('superadmin.admin.show', $admin->id) }}" class="text-xs font-bold text-slate-800 dark:text-white hover:text-brand-purple transition-colors">{{ $admin->username }}</a>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400">{{ $admin->email }}</td>
                                        <td class="px-6 py-4">
                                            @if($admin->role === 'superadmin')
                                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[9px] font-extrabold uppercase tracking-wide bg-brand-purple-light dark:bg-brand-purple/10 text-brand-purple">
                                                    <i class="ri-shield-star-line text-[9px]"></i> Superadmin
                                                </span>
                                            @else
                                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[9px] font-extrabold uppercase tracking-wide bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">
                                                    <i class="ri-admin-line text-[9px]"></i> Admin
                                                </span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4">
                                                ...
                                                @forelse($permissions as $permission)
                                                    @php
                                                        $permConfig = match($permission) {
                                                            'kelola_mentor' => ['icon' => 'ri-user-star-line', 'label' => 'Mentor', 'class' => 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400'],
                                                            'kelola_mentee' => ['icon' => 'ri-team-line', 'label' => 'Mentee', 'class' => 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400'],
                                                            'kelola_course' => ['icon' => 'ri-book-open-line', 'label' => 'Course', 'class' => 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400'],
                                                            default => ['icon' => 'ri-key-line', 'label' => $permission, 'class' => 'bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-slate-300']
                                                        };
                                                    @endphp
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold {{ $permConfig['class'] }}">
                                                        <i class="{{ $permConfig['icon'] }} text-[9px]"></i> {{ $permConfig['label'] }}
                                                    </span>
                                                @empty
                                        ...
                                        @if(auth()->user()->role === 'superadmin')
                                            <td class="px-6 py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    <button class="px-3 py-1.5 bg-slate-100 hover:bg-red-50 dark:bg-slate-850 dark:hover:bg-red-500/10 hover:text-red-650 text-slate-600 dark:text-slate-400 text-[10px] font-bold rounded-lg transition-colors flex items-center gap-1 toggle-status"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ auth()->user()->role === 'superadmin' ? 6 : 5 }}" class="py-10 text-center text-slate-400 dark:text-slate-600">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterTable() {
            ...
            tableRows.forEach(row => {
                const name = row.querySelector('.text-slate-800')?.textContent.toLowerCase() || '';
        ...
        function openStatusModal(id, name, action, row) {
            currentAdminId = id;
        ...
        if (confirmBtn) {
            confirmBtn.addEventListener('click', async function() {
                ...
                try {
                    const newStatus = currentAction === 'nonaktif' ? 'nonaktif' : 'aktif';

                    const response = await fetch(`/superadmin/dashboard/admin/${currentAdminId}/update-status`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (result.success && currentRow) {
                        ...
                        if (newStatus === 'nonaktif') {
                            ...
                            toggleBtn.className = 'px-3 py-1.5 bg-slate-100 hover:bg-emerald-50 dark:bg-slate-850 dark:hover:bg-emerald-500/10 hover:text-emerald-500 text-slate-600 dark:text-slate-400 text-[10px] font-bold rounded-lg transition-colors flex items-center gap-1 toggle-status';
                            ...
                            toggleBtn.className = 'px-3 py-1.5 bg-slate-100 hover:bg-red-50 dark:bg-slate-850 dark:hover:bg-red-500/10 hover:text-red-500 text-slate-600 dark:text-slate-400 text-[10px] font-bold rounded-lg transition-colors flex items-center gap-1 toggle-status';
