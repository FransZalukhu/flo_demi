...
export function createSparkline(selector, data, options = {}) {
  ...
  // Calculate min/max with proper handling of zero values
  const dataMin = Math.min(...data);
  const dataMax = Math.max(...data);
  
  // If all values are the same, add small range for visualization
  const min = dataMin === dataMax ? 0 : dataMin * 0.8;
  const max = dataMin === dataMax ? (dataMax + 1) * 1.2 : dataMax * 1.1;
      ...
      datasets: [
        {
          ...
          backgroundColor: `${options.color || colors.primary}15`,
          borderWidth: 2,
          pointRadius: 0,
          pointHoverRadius: 3,
          tension: 0.4,
